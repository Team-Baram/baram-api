...
export class ContactRepository {
    ...
    async findByUserId(userId: string): Promise<Contact[] | null> {
        try {
            return await this.repo.find({ where: { user: { id: userId }}, order: { createdAt: 'DESC' } })
        } catch (err) {
            throw new InternalServerErrorException('findByUserId in ContactRepository')
        }
    }
